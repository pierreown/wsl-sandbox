...
INI_GET() { awk -F'=' -v SECTION="$2" -v KEY="$3" "$AWK_GET_MODULE" "$1"; }
INI_SET() { awk -F'=' -v SECTION="$2" -v KEY="$3" -v NEW_VALUE="$4" "$AWK_SET_MODULE" "$1"; }
FMT() {
    FMT_TYPE="$1"
    case "$FMT_TYPE" in
...

{
    ...
    echo "type wsl-enter && exec wsl-enter" >/etc/profile.d/99-wsl-enter.sh
